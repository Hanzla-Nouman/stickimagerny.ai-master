...
import { callFastSVD, generateUUID } from "@/utils";
...
function Image2Video() {
  ...
  const [motionBucketId, setMotionBucketId] = useState();
  const [condAug, setCondAug] = useState();
  ...
  async function handleGenerate() {
    if (userCredits < 4) {
      ...
      // const baseImageUrl = baseImageResult.url; // adjust as needed

      try {
        const generateId = generateUUID();

        const result = await callFastSVD(
          finalImageUrl,
          motionBucketId,
          condAug,
          steps,
        ...
        const newVideoUrl = { url: responseVideoUrl };
      ...
      <div className="flex flex-col md:flex-row">
        <div className="w-full md:w-1/4  overflow-y-auto bg-slate-50 p-4 border rounded-md flex-col relative">
          <div>
            ...
            <p className="text-xs text-default-500 mb-2">Choose an image to add motion to</p>

            <div className="mb-8">
              
              <ImagePicker fileName={baseImage} setFileName={setBaseImage} controlImage={controlImage} setControlImage={setControlImage} fileInputRef={fileInputRef}/>              
            </div>
            <div className="mb-8">
            ...
            <Button
              ...
              isDisabled={(baseImage || controlImage) && !loading ? false : true}
        ...
        <div className=" w-full md:w-3/4 p-4 overflow-y-auto h-screen  bg-white bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]">
          ...
          <div className="flex pb-4 justify-center items-center h-half w-1/2 m-auto bg-glass rounded-lg shadow-lg overflow-hidden">
          ...
          {videoUrls &&
              videoUrls.map((video, index) => (
                <>
                  <video key={index} src={video.url} controls autoPlay loop className="rounded-lg max-w-[500px] max-h-[500px] mb-8" />
                </>
              ))}
                </div>
        </div>
